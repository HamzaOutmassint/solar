...
import { DataLayersResponse } from '../types/solar';
import { LatLng } from '../types/solar';
import { Bounds } from '../types/solar';
import { Date } from '../types/solar';
...
export async function downloadGeoTIFF(url: string, apiKey: string): Promise<GeoTiff> {
  ...
  return {
    ...
    //   ),
    rasters: Array.from(Array(rasters.length).keys()).map((i) =>
      Array.from(rasters[i] as geotiff.TypedArray)
    ),
